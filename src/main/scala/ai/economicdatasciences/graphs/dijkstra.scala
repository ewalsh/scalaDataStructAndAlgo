...
class MyDirectedGraph[V](adjacencyList: Map[V, List[V]]) extends MyGraph[V] {
  ...
  override def edges: List[(V, V)] = adjacencyList
    .flatMap({ case (v, neighbors) =>
      neighbors.map(x => (v, x))
    })
    .toList
...
class MyWeightedGraph[V](adjacencyList: Map[V, List[MyWeightedEdge[V]]])
    extends MyGraph[V] {
  override def vertices: List[V] = adjacencyList.keys.toList

  override def edges: List[(V, V)] = adjacencyList
    .flatMap({ case (v, edgeList) =>
      edgeList.map(e => v -> e.dest)
    })
    .toList
  ...
  override def addEdge(a: V, b: V): MyWeightedGraph[V] =
    addEdge(a, new MyWeightedEdge(b, weight = 0))
  ...
  def neighborsWithWeight(x: V): List[MyWeightedEdge[V]] =
    adjacencyList.getOrElse(x, Nil).toList
}
