...
object KnuthMorrisPrattApp {
  ...
  def kmpSubstringSearch(myWords: String, myData: String): Int = {
    val prefixTabl = prefixTable(myWords)

    myData.indices
      .foldLeft(-1, 0)({
        case ((foundIndex, x), i) if foundIndex > 0 => (foundIndex, 0)
        case ((foundIndex, x), i) => {
          val stepsX = Stream.iterate(x)(x => prefixTabl(x - 1))
      ...
      .foldLeft(0, Vector(0))({
        case ((initialValue, prefixT), currentCharacter) => {
          val lowerValue = Stream
            .iterate(initialValue)(initialValue => {
              prefixT(initialValue - 1)
            })
            .find(initialValue => {
            ...
            .get
          val newValue = if (searchString(lowerValue) == currentCharacter) {
            lowerValue + 1
          } else {
            lowerValue
          }
